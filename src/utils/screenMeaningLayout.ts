/**
 * Screen Meaning Layout Utilities
 *
 * Calculates word-by-word overflow for meaning indicators in on-screen rendering
 * Implements the same priority-based overflow logic as the PDF version
 *
 * Priority Order: English (highest) > Han-Viet > Vietnamese (lowest)
 */

/**
 * Estimate text width for screen rendering
 * Uses character-based estimation (similar to PDF version)
 *
...
function fitWordsInZone(
  ...
  const fitted: string[] = [];
  let currentWidth = 0;
  const separatorWidth = estimateTextWidth(separator, fontSizePx);
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    // First word always fits (will be truncated by CSS if needed)
    if (i === 0) {
      fitted.push(word);
    ...
    if (currentWidth + addedWidth <= maxWidthPx) {
      fitted.push(word);
...
/**
 * Calculate meaning layout for screen with overflow
 * Implements priority-based overflow (English > Han-Viet > Vietnamese)
 */
export interface ScreenMeaningLayout {
  english: {
    top: string;
    right: string;
    bottom: string;
...
export interface ScreenMeaningConfig {
  ...
  topZoneWidthPx: number;    // Width of TOP zone in pixels (horizontal text)
  sideZoneHeightPx: number;  // Height of LEFT/RIGHT zones in pixels (vertical text height)
  bottomZoneWidthPx: number; // Width of BOTTOM zone in pixels (horizontal text)
  englishFontSizePx: number;
  vietnameseFontSizePx: number;
  hanVietFontSizePx: number;
}

export function calculateScreenMeaningLayout(config: ScreenMeaningConfig): ScreenMeaningLayout {
  const layout: ScreenMeaningLayout = {
  ...
  const englishPrimary = config.showEnglish && config.englishMeanings.length > 0
    ? fitWordsInZone(config.englishMeanings, config.topZoneWidthPx, config.englishFontSizePx)
    : { fitted: [], overflow: [] };
  ...
  if (config.showVietnamese && config.vietnameseMeanings.length > 0) {
    if (config.showEnglish) {
      ...
      vietnamesePrimary = fitWordsInZone(config.vietnameseMeanings, config.sideZoneHeightPx, config.vietnameseFontSizePx);
    } else {
      // English is OFF: Vietnamese primary = TOP zone
      vietnamesePrimary = fitWordsInZone(config.vietnameseMeanings, config.topZoneWidthPx, config.vietnameseFontSizePx);
    }
  } else {
  ...
  const hanVietPrimary = config.showHanViet && config.hanVietMeanings.length > 0
    ? fitWordsInZone(config.hanVietMeanings, config.bottomZoneWidthPx, config.hanVietFontSizePx)
    : { fitted: [], overflow: [] };
  ...
  if (config.showEnglish && englishRemaining.length > 0) {
    ...
    if (!layout.vietnamese.right) {
      const fit = fitWordsInZone(englishRemaining, config.sideZoneHeightPx, config.englishFontSizePx);
    ...
    if (englishRemaining.length > 0 && !layout.hanViet.bottom) {
      const fit = fitWordsInZone(englishRemaining, config.bottomZoneWidthPx, config.englishFontSizePx);
  ...
  if (config.showHanViet && hanVietRemaining.length > 0) {
    ...
    if (!layout.english.top && !layout.vietnamese.top) {
      const fit = fitWordsInZone(hanVietRemaining, config.topZoneWidthPx, config.hanVietFontSizePx);
    ...
    if (hanVietRemaining.length > 0 && !layout.vietnamese.right && !layout.english.right) {
      const fit = fitWordsInZone(hanVietRemaining, config.sideZoneHeightPx, config.hanVietFontSizePx);
  ...
  if (config.showVietnamese && vietnameseRemaining.length > 0) {
    if (config.showEnglish) {
      ...
      if (!layout.hanViet.bottom && !layout.english.bottom) {
        const fit = fitWordsInZone(vietnameseRemaining, config.bottomZoneWidthPx, config.vietnameseFontSizePx);
      ...
      if (vietnameseRemaining.length > 0 && leftZoneAvailable && !layout.english.left && !layout.hanViet.left) {
        const fit = fitWordsInZone(vietnameseRemaining, config.sideZoneHeightPx, config.vietnameseFontSizePx);
